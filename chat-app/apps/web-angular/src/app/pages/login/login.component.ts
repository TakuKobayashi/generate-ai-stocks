      ...
      <div class="auth-box">
        ...
        <div class="auth-card">
          ...
          @if (tab() === 'password') {
            ...
            <div>
              <div class="field">
                <label class="label" for="passkey-email">メールアドレス（任意）</label>
                <input id="passkey-email" class="input" type="email" placeholder="you@example.com"
                  [(ngModel)]="email" name="passkeyEmail" autocomplete="email" />
...
export class LoginComponent {
  ...
  async loginWithPassword() {
    ...
    try {
      ...
      this.error.set(err?.error?.error ?? 'ログインに失敗しました');
    } finally {
  ...
  async loginWithPasskey() {
    ...
    try {
      ...
      const res = await firstValueFrom(
        this.http.post<AuthResponse>('/api/auth/passkey/auth/verify', {
          challengeId,
          response,
        })
      );
      ...
      this.error.set(err?.error?.error ?? 'パスキー認証に失敗しました');
    } finally {
      this.loading.set(false);
    }
  }
