...
@Component({
  ...
  template: `
    <div class="auth-page">
      <div class="auth-box">
        <div class="auth-logo"><h1>ChatApp</h1><p>Angular — リアルタイムチャット</p></div>
        <div class="auth-card">
          <h2>ログイン</h2>
          <div class="tabs">
          ...
          @if (tab() === 'password') {
            <form (ngSubmit)="loginWithPassword()">
              ...
              <div class="field" style="margin-top:16px">
                ...
                <input id="pw" class="input" type="password" placeholder="••••••••" [(ngModel)]="password" name="password" required autocomplete="current-password" />
              </div>
            ...
            <div>
              <div class="field">
                ...
                <input id="pk-email" class="input" type="email" placeholder="you@example.com" [(ngModel)]="email" name="pkEmail" autocomplete="email" />
                <span style="font-size:12px;color:var(--text-3);margin-top:6px;display:block">省略するとデバイスに保存された全パスキーから選択できます</span>
              </div>
  ...
  async loginWithPasskey() {
    ...
    try {
      const { options, challengeId } = await firstValueFrom(
        this.http.post<{ options: PublicKeyCredentialRequestOptionsJSON; challengeId: string }>(
          '/api/auth/passkey/auth/options', { email: this.email || undefined }
        )
      );
