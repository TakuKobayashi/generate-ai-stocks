        ...
        <div class="auth-card">
          ...
          <form (ngSubmit)="register()">
            <div class="field">
              ...
              <input id="displayName" class="input" type="text" placeholder="山田 太郎"
                [(ngModel)]="displayName" name="displayName" required autocomplete="name" />
            </div>
            ...
            <div class="field" style="margin-top:16px">
              <label class="label" for="password">パスワード（8文字以上）</label>
          ...
          <div class="auth-footer">
            すでにアカウントをお持ちの方は <a routerLink="/login">ログイン</a>
          </div>
        </div>
      </div>
    </div>
  ...
  async registerPasskey() {
    ...
    try {
      const { options, challengeId } = await firstValueFrom(
        this.http.post<{ options: any; challengeId: string }>(
      ...
      await firstValueFrom(
        this.http.post('/api/auth/passkey/register/verify', { challengeId, response })
      );
      this.success.set('パスキーを登録しました！');
    } catch (err: any) {
      this.error.set(err?.error?.error ?? 'パスキー登録に失敗しました');
    } finally {
