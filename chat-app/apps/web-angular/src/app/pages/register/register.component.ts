...
@Component({
  ...
  template: `
    <div class="auth-page">
      <div class="auth-box">
        <div class="auth-logo"><h1>ChatApp</h1><p>新規アカウント作成</p></div>
        <div class="auth-card">
          <h2>新規登録</h2>
          ...
          <form (ngSubmit)="register()">
            <div class="field">
              ...
              <input id="dn" class="input" type="text" placeholder="山田 太郎" [(ngModel)]="displayName" name="displayName" required autocomplete="name" />
            </div>
            <div class="field" style="margin-top:16px">
              <label class="label" for="email">メールアドレス</label>
              <input id="email" class="input" type="email" placeholder="you@example.com" [(ngModel)]="email" name="email" required autocomplete="email" />
            </div>
            <div class="field" style="margin-top:16px">
              <label class="label" for="pw">パスワード（8文字以上）</label>
              <input id="pw" class="input" type="password" placeholder="••••••••" [(ngModel)]="password" name="password" required minlength="8" autocomplete="new-password" />
            </div>
            ...
            <div>
              ...
              <button class="passkey-btn" (click)="registerPasskey()" [disabled]="passkeyLoading()" type="button">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/><path d="M16 11l1.5 1.5L20 10"/>
                </svg>
...
export class RegisterComponent {
  ...
  loading = signal(false); passkeyLoading = signal(false);

  async register() {
    this.error.set(''); this.loading.set(true);
    try {
      const res = await firstValueFrom(this.http.post<AuthResponse>('/api/auth/register', { email: this.email, password: this.password, displayName: this.displayName }));
  ...
  async registerPasskey() {
    ...
    if (!user) return;
    this.error.set(''); this.success.set(''); this.passkeyLoading.set(true);
    try {
      const { options, challengeId } = await firstValueFrom(
        this.http.post<{ options: PublicKeyCredentialCreationOptionsJSON; challengeId: string }>(
          '/api/auth/passkey/register/options', { userId: user.id, displayName: user.displayName }
        )
      );
