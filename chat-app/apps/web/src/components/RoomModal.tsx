    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <form onSubmit={handleSubmit}>
          <div className="field">
            <label className="label" htmlFor="room-name">ルーム名</label>
            <input id="room-name" className="input" type="text" placeholder="general" value={name} onChange={(e) => setName(e.target.value)} required />
          </div>
          <div className="field">
            <label className="label" htmlFor="room-desc">説明（任意）</label>
            <input id="room-desc" className="input" type="text" placeholder="このルームの目的" value={description} onChange={(e) => setDescription(e.target.value)} />
          </div>
