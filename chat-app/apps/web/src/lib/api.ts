...
export const api = {
  auth: {
    ...
    passkeyAuthVerify: (data: { challengeId: string; response: unknown }) =>
      request<AuthResponse>("/api/auth/passkey/auth/verify", {
        method: "POST",
        body: JSON.stringify(data),
      }),
  },
  ...
  rooms: {
    ...
    create: (data: { name: string; description?: string }) =>
      request<{ data: Room }>("/api/rooms", {
        method: "POST",
        body: JSON.stringify(data),
      }),
    update: (id: string, data: { name?: string; description?: string }) =>
      request<{ data: Room }>(`/api/rooms/${id}`, {
        method: "PUT",
        body: JSON.stringify(data),
      }),
    delete: (id: string) =>
    ...
    messages: (roomId: string) =>
      request<{ data: Message[] }>(`/api/rooms/${roomId}/messages`),
  },
};

// Type shims for WebAuthn JSON types (available in modern browsers)
declare global {
