...
// WebSocket endpoint — proxies to Durable Object per room
app.get("/ws/:roomId", authMiddleware, async (c) => {
  ...
  url.searchParams.set("displayName", encodeURIComponent(displayName));
...
// Fallback: serve static assets (Next.js)
app.get("*", async (c) => {
