...
export async function saveUser(env: Env, user: User & { passwordHash?: string | null }): Promise<void> {
  await Promise.all([env.USERS_KV.put(`user:${user.id}`, JSON.stringify(user)), env.USERS_KV.put(`email:${user.email.toLowerCase()}`, user.id)]);
}

export interface StoredPasskey { id: string; userId: string; credentialId: string; publicKey: number[]; counter: number; deviceType: string; backedUp: boolean; transports?: string[]; createdAt: string }
export async function getPasskeyByCredentialId(env: Env, credentialId: string): Promise<StoredPasskey | null> { return env.USERS_KV.get(`passkey:cred:${credentialId}`, "json"); }
...
export async function savePasskey(env: Env, p: StoredPasskey): Promise<void> {
  ...
  await Promise.all([env.USERS_KV.put(`passkey:${p.id}`, JSON.stringify(p)), env.USERS_KV.put(`passkey:cred:${p.credentialId}`, JSON.stringify(p)), env.USERS_KV.put(`user:${p.userId}:passkeys`, JSON.stringify(ids))]);
}
export async function updatePasskeyCounter(env: Env, p: StoredPasskey, counter: number): Promise<void> {
  const u = { ...p, counter };
  await Promise.all([env.USERS_KV.put(`passkey:${p.id}`, JSON.stringify(u)), env.USERS_KV.put(`passkey:cred:${p.credentialId}`, JSON.stringify(u))]);
}

export interface StoredChallenge { id: string; challenge: string; userId?: string; expiresAt: string }
...
export async function getChallenge(env: Env, id: string): Promise<StoredChallenge | null> { return env.CHALLENGES_KV.get(`challenge:${id}`, "json"); }
export async function deleteChallenge(env: Env, id: string): Promise<void> { await env.CHALLENGES_KV.delete(`challenge:${id}`); }

export interface StoredSession { id: string; userId: string; expiresAt: string; createdAt: string }
...
export async function getSession(env: Env, id: string): Promise<StoredSession | null> { return env.SESSIONS_KV.get(`session:${id}`, "json"); }
export async function deleteSession(env: Env, id: string): Promise<void> { await env.SESSIONS_KV.delete(`session:${id}`); }
...
export async function getRoomById(env: Env, id: string): Promise<Room | null> { return env.ROOMS_KV.get(`room:${id}`, "json"); }
